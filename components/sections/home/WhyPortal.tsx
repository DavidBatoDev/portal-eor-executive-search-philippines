    ...
    <Section id="why" bg="cream">
      <Container>
        <div className="grid items-start gap-[clamp(2rem,5vw,5rem)] lg:grid-cols-2">
          <Reveal className="max-w-190 self-start lg:sticky lg:top-25">
            <Eyebrow className="mb-[1.1rem]">{whyPortal.eyebrow}</Eyebrow>
            <h2>{whyPortal.heading}</h2>
            <p className="lead mt-5">{whyPortal.lead}</p>
          ...
          <Reveal delay={1} className="grid gap-[clamp(1.2rem,2.2vw,1.5rem)] sm:grid-cols-2">
            {whyPortal.cards.map((c) => (
              ...
              >
                ...
                <span className="mb-[1.2rem] grid h-12.5 w-12.5 place-items-center rounded-[13px] border border-[#f0e3c4] bg-gold-tint text-gold-deep">
                  <Icon name={c.icon} className="h-6 w-6" />
                </span>
                <h4 className="mb-2 text-[1.08rem]">{c.title}</h4>
